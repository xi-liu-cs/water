...
sealed class MeshBuilder : System.IDisposable
{
    ...
    public MeshBuilder(int x, int y, int z, int budget, ComputeShader compute, Material material)
      => Initialize((x, y, z), budget, compute, material);

    public MeshBuilder(Vector3Int dims, int budget, ComputeShader compute, Material material)
      => Initialize((dims.x, dims.y, dims.z), budget, compute, material);
    ...
    void Initialize((int, int, int) dims, int budget, ComputeShader compute, Material mat)
    {
        _grids = dims;
